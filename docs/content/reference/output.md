...
describes what users get. Pick a format with `-o`, or let cacm choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cacm <command> -o table   # aligned columns for reading
cacm <command> -o jsonl   # one JSON object per line, for piping
cacm <command> -o json    # a single JSON array
cacm <command> -o csv     # spreadsheet friendly
cacm <command> -o tsv     # tab-separated
cacm <command> -o url     # just the URL column
cacm <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cacm <command> --fields id,title,url
```
...
```bash
cacm <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cacm <command>            # a table, because this is a terminal
cacm <command> | wc -l    # JSONL, because this is a pipe
```
