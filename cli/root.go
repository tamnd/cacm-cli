// Package cli builds the cacm command tree on top of the cacm library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: cacm.DefaultConfig()}

	root := &cobra.Command{
		Use:   "cacm",
		Short: "Read Communications of the ACM articles and blogs.",
		Long: `cacm reads Communications of the ACM (CACM) through the public RSS and Atom
feeds at cacm.acm.org. No API key or authentication is required. It returns
records as table, JSON, JSONL, CSV, TSV, or URLs.

cacm is an independent tool and is not affiliated with the Association for
Computing Machinery.`,
		SilenceUsage:  true,
		SilenceErrors: true,
